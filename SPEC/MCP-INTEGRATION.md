...
**关键设计决策**: MCP 模块不直接启动或管理 AI CLI 进程，而是生成命令字符串供 Claude Code 执行。

**原理**:
1. **代码复用**: MCP 生成的 `agent` 命令通过现有的 `supervisor::execute_cli()` 执行
2. **避免重复**: 不在 MCP 中重复实现 Provider 配置、环境变量注入、TaskRegistry 注册等逻辑
3. **统一流程**: 无论通过 MCP 还是 CLI 启动，都使用同一套 supervisor 逻辑
4. **简化维护**: MCP 只负责命令字符串生成，supervisor 负责所有执行细节
...
#### AI CLI 命令生成工具
- **`start_concurrent_tasks`**: 生成并发执行多个 AI CLI 的 Bash 命令字符串
  - 返回: `start_command` (并发启动命令) 和 `wait_command` (等待命令)
  - 不直接启动进程，由 Claude Code 通过 Bash 工具执行
- **`get_task_command`**: 生成单个 AI CLI 的启动命令字符串
  - 返回: `agent <ai_type> 'task'` 格式的命令
  - 不直接启动进程，由 Claude Code 通过 Bash 工具执行
- **`start_ai_cli`**: (已移除) 该方法不再提供，使用上述命令生成工具代替
...
**步骤2: Agentic-Warden 生成并发启动命令**
- 为每个任务生成 `agent <ai_type>` 命令字符串
- 使用 `&` 拼接为并发执行的 Bash 命令
- 立即返回命令字符串（不直接启动进程）
...
```json
{
  "success": true,
  "task_count": 3,
  "start_command": "agent codex -p openrouter '分析代码A' & agent gemini '生成文档B' & agent codex '重构模块C' &",
...
**步骤4: Claude Code 执行并发启动命令**
```bash
# Claude Code使用Bash工具执行并发启动（后台）
$ agent codex -p openrouter '分析代码A' & agent gemini '生成文档B' & agent codex '重构模块C' &
# 每个 agent 命令通过 supervisor::execute_cli() 启动
# supervisor 自动注册到 TaskRegistry
```

**步骤5: Claude Code 执行等待命令**
```bash
