...
- 返回 "agentic-warden wait" 命令
- Claude Code 执行 wait 命令等待所有任务完成
- 适用于：多个任务并发执行
...
- **`start_concurrent_tasks`**: 并发启动多个 AI CLI 任务
  ...
  - 返回: `wait_command` (agentic-warden wait 命令)
  - 通过 supervisor::execute_cli() 复用现有实现
  - 每个任务自动注册到 TaskRegistry
- **`get_task_command`**: 获取单个 AI CLI 的启动命令字符串
...
```json
{
  ...
  "message": "Started 3 concurrent AI CLI tasks in background. Execute wait_command to wait for completion."
}
```
...
$ agentic-warden wait --timeout 12h
```
