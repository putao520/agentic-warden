...
### TUI 组件使用原则（强制要求）
- **禁止创建自定义 TUI 组件**：必须直接使用 ratatui 提供的原生组件（`Paragraph`、`Block`、`List`、`Gauge`、`Table`、`Chart` 等）
- **禁止封装 ratatui 组件**：避免不必要的抽象层，不得创建 `ErrorDialog`、`ConfirmDialog`、`ProgressWidget` 等自定义组件类
- **遵循 KISS 原则**：通过组合现有组件构建界面，而不是创建新的组件类型
- **正确做法示例**：使用 `Paragraph::new(text).block(Block::default().borders(Borders::ALL))` 等组合方式直接构建 UI
- **违反行为明确禁止**：任何试图抽象、封装或扩展 ratatui 组件的行为都是不允许的
