...
**版本支持矩阵**（跨 2.1.195-201）：

| 版本 | max-token | AntiTelemetry | AntiSpy(时区 B) | AntiSpy(逃生口 A) | AntiPromptBias | AntiAtis |
|------|-----------|---------------|-----------------|-------------------|----------------|----------|
| 2.1.195 | ✅ 正则放宽 | ✅ 27B 字面量 | ✅ 48B 字面量 | ✅ `if(Oe.xxx)`→`if(1)` 55B regex | ✅ 语义正则 63B（`if(g7())`） | — (195 无 atis) |
| 2.1.196 | ✅ 正则放宽 | ✅ | ✅ 48B 字面量 | ✅ `if(Oe.xxx)`→`if(1)` 55B regex | ✅ 语义正则 63B（`if(F7())`） | ✅ 80B（S6r/P0） |
| 2.1.197 | ✅ 正则放宽 | ✅ | ✅ 48B 字面量 | ✅ `if(Oe.xxx)`→`if(1)` 55B regex | ✅ 语义正则 63B（`if(j7())`） | ✅ 80B（H6r/AI） |
| 2.1.198 | ✅ 正则放宽 | ✅ | ✅ 48B 字面量 | ✅ `if(Pe.xxx)`→`if(1)` 55B regex | ✅ 语义正则 63B（`if(dX())`） | ✅ 80B（pYr/I0） |
| 2.1.199 | ✅ 正则放宽 | ✅ | ✅ 48B 字面量 | ✅ `if(De.xxx)`→`if(1)` 55B regex | ✅ 语义正则 63B（`if(qX())`） | ✅ 80B（SXr/q0） |
| 2.1.201 | ✅ 正则放宽 | ✅ | ✅ 48B 字面量 | ✅ `if(ke.xxx)`→`if(1)` 55B regex | ✅ 语义正则 63B（`if(dJ())`） | ✅ 80B（jJr/nR） |

说明：
- max-token 正则放宽 `var \w+=200000,\w+=200000[^;]*;` 兼容 195(5元素)/196(5元素)/197(6元素含无值变量)/198(4元素)/199(4元素)/201(4元素)
- AntiSpy 逃生口 `<OBJ>` 跨版本通配（Oe/Pe/De/ke），regex 字面量模式跨 195-201 通用
- AntiSpy 时区 Patch B 是函数体字面量，跨版本稳定（非 minified 变量名）
- AntiPromptBias 语义正则 `if(\w+())n.push("**Provider context:**` 通配 g7/F7/j7/dX/qX/dJ 等条件函数名
- AntiAtis 语义正则通配 atis 提取函数名（3 字符）+ bootstrap 函数名（2 字符），跨 196-201 通用（195 无此机制）
- **2.1.195+ native binary 仅 GCS 发布 linux-x64/linux-arm64，macos/windows 不再发布**
