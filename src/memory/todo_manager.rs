...
impl TodoManager {
    ...
    pub async fn create_todo(
        ...
        if let Some(desc) = &todo.description {
            ...
            self.storage
                .add_memory(&content_with_desc, metadata)
                .await?;
        } else {
    ...
    pub async fn update_todo_status(
        ...
        let results = self
            .storage
            .search(&format!("TODO id: {}", todo_id), Some(1), Some(0.9))
            .await?;
        ...
        if let Some(result) = results.first() {
            ...
            let content = format!(
                "TODO: {}",
                result.point.content.replace("TODO:", "UPDATED TODO:")
            );
            self.storage.add_memory(&content, metadata).await?;
        }
    ...
    pub async fn search_todos(
        ...
        let results = self
            .storage
            .search(&search_query, Some(20), Some(0.5))
            .await?;

        for result in results {
            if result.point.content.starts_with("TODO:")
                || result.point.content.starts_with("UPDATED TODO:")
            {
                let mut metadata = result.point.metadata.clone();

                // Extract status as owned string to avoid borrowing issues
                let status = metadata
                    .remove("status")
                ...
                // Extract priority as owned string
                ...
                // Extract description as owned string
                let description = metadata
                    .remove("description")
                    .and_then(|v| v.as_str().map(|s| s.to_string()));

                // Extract updated_at as owned string
    ...
    pub async fn get_todos_by_session_id(
        ...
        for result in results {
                ...
                || result.point.content.starts_with("UPDATED TODO:")
            {
                ...
                // Extract status as owned string
                let status = metadata
                    .remove("status")
                    .and_then(|v| v.as_str().map(|s| s.to_string()))
                    .and_then(|s| match s.as_str() {
