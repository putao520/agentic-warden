...
use super::{
    ...
    vector_store::{VectorStore, SearchResult},
    semantic_search::SemanticSearch,
    todo_manager::{TodoManager, TodoItem},
};
...
impl MemoryManager {
    ...
    pub async fn test_connections(&self) -> anyhow::Result<HashMap<String, bool>> {
        ...
        let ollama_ok = self.embedding_service.test_connection().await.unwrap_or(false);
    ...
    pub async fn store_conversation(
        ...
        final_metadata.insert("type".to_string(), serde_json::Value::String("conversation".to_string()));
        final_metadata.insert("timestamp".to_string(), serde_json::Value::String(chrono::Utc::now().to_rfc3339()));

        self.semantic_search.add_memory(content, final_metadata).await
    }
    ...
    pub async fn extract_and_store_todos(
        &self,
        text: &str,
        ...
        for todo in &todos {
            ...
            metadata.insert("todo_id".to_string(), serde_json::Value::String(todo.id.clone()));

            let content = format!("TODO: {}", todo.title);
            if let Some(desc) = &todo.description {
                let content_with_desc = format!("{}\n\n{}", content, desc);
                self.semantic_search.add_memory(&content_with_desc, metadata.clone()).await?;
            } else {
                self.semantic_search.add_memory(&content, metadata.clone()).await?;
            }
        }
    ...
    pub async fn create_todo(
        ...
        self.todo_manager.create_todo(title, description, session_id).await
    }
    ...
    pub async fn get_todos_by_session_id(
        ...
        self.todo_manager.get_todos_by_session_id(session_id, status_filter).await
    }
    ...
    pub async fn get_memory_stats(&self) -> anyhow::Result<HashMap<String, serde_json::Value>> {
        ...
        let all_memories = self.semantic_search.search("", Some(1000), Some(0.0)).await?;
        ...
        stats.insert("total_memories".to_string(), serde_json::Value::Number(serde_json::Number::from(all_memories.len())));
        stats.insert("conversations".to_string(), serde_json::Value::Number(serde_json::Number::from(conversations)));
        stats.insert("todos".to_string(), serde_json::Value::Number(serde_json::Number::from(todos)));
        stats.insert("others".to_string(), serde_json::Value::Number(serde_json::Number::from(others)));
